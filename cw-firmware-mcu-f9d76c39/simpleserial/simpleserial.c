...
#elif SS_VER == SS_VER_1_0

#undef MAX_SS_LEN
#define MAX_SS_LEN BUFSIZ
#undef MAX_SS_CMDS
#define MAX_SS_CMDS 95  /* the number of printable characters in ASCII */

typedef struct ss_cmd
{
	char c;
	uint8_t (*fp)(uint8_t*, uint32_t);
} ss_cmd;

static ss_cmd commands[MAX_SS_CMDS];

int simpleserial_addcmd(char c, uint8_t (*fp)(uint8_t*, uint32_t))
{
	if(num_commands >= MAX_SS_CMDS)
		return 1;

	commands[num_commands].c   = c; /* any printable char is sufficient */
	commands[num_commands].fp  = fp;
	num_commands++;
...
void simpleserial_get(void)
{
	char line_buf[2*MAX_SS_LEN + 2];  /* +2 for cmd and \n */
	uint8_t data_buf[MAX_SS_LEN];
	char c;
	unsigned int len;
	int cmd;
	...
	if(hex_decode(len, ascii_buf, data_buf))
		return;
#else
	/* Read entire line at once until \n */
	...
	/* Check if we got at least the command character */
	if (line_len < 1)
		return;

	/* Extract command character (first byte) */
	c = line_buf[0];
	line_len--;  /* Skip cmd char */
	...
	/* Calculate data length (rest of the line after cmd char) */
	len = line_len / 2;  /* /2 for hex pairs */

	/* Decode hex data (skip first character which is the command) */
	if (hex_decode(len, line_buf + 1, data_buf))  /* +1 to skip cmd char */
		return;  /* Decode error */
#endif

	/* Callback */
	uint8_t ret = commands[cmd].fp(data_buf, len);
	printf("%c%02x%c", 'z', ret, '\n');
}


#else  /* SS_VER == SS_VER_1_1 */
