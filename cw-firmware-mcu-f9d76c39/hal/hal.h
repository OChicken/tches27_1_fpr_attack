...
void putch(char c);  // Send single char, blocking
char getch(void);  // Receive single char, blocking
...
#define HAL_ibex  30
    ...
    #include "mps2/CMSDK_CM4.h"
#elif HAL_TYPE == HAL_avr
    #include <avr/io.h>
...
/*****************************************************************************/
/*                   Below are from /pqm4/mupq/common/hal.h                  */
/*****************************************************************************/
...
#if HAL_TYPE != HAL_mps2
#define HAL_CYCCNT() (__extension__ ({ \
	__asm__ volatile("" ::: "memory"); \
        uint32_t _t = DWT->CYCCNT; \
        __asm__ volatile("" ::: "memory"); \
        _t; }))
#else
uint64_t HAL_CYCCNT(void);
#endif
