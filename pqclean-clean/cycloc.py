...
The values below are for OPT=O3:

  leakage   addr_global  rel_start  rel_end
  --------  -----------  ---------  -------
  scaled    0x08000e00   0x00       0x18e
  mul       0x0800121c   0x28       0x72
  shift55   0x0800121c   0x76       0xbe

  expected_delta (addr1-addr0 period of ETM events):
  leakage   Falcon512              Falcon1024
  --------  ---------------------  ----------------------
  scaled    [160]                  [160]
  mul       [116, 338, 113, 1317]  [116, 337, 113, 1314]
  shift55   [116, 338, 113, 1317]  [116, 337, 113, 1314]
...
def rm_etm_abnormal(loc, expected_deltas, tol=10):
    ...
    for i in range(1, len(loc)):
        ...
        if abs(delta - exp) <= tol:  # >
            ...
            # one event was missed: insert the synthetic entry, then the real one
...
if __name__ == '__main__':
    ...
    # loc_file ################################################################
    if KEY_TYPE == 'profile':
        LOC_FILE = f"{OPT}/data-falcon{FALCON_N}/profile/loc_{LEAKAGE}.pkl"
    else:
        LOC_FILE = f"{OPT}/data-falcon{FALCON_N}/attack/loc_{LEAKAGE}_{KEY_TYPE}.pkl"
    ...
    # set CW ##################################################################
    scope, target = hardware_setup(f"{OPT}/build-stm32f4/{name}",
